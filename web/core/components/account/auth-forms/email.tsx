...
export const AuthEmailForm: FC<TAuthEmailForm> = observer((props) => {
  ...
  const emailError = useMemo(
    () => (email && !checkEmailValidity(email) ? { email: "Email non valida" } : undefined),
    [email]
  );
  ...
  return (
    <form onSubmit={handleFormSubmit} className="mt-5 space-y-4">
      <div className="space-y-1">
        ...
        <div
          className={cn(
            ...
            !isFocused && Boolean(emailError?.email) ? `border-red-500 bg-red-50` : `border-onboarding-border-100`
          )}
          tabIndex={-1}
        ...
        >
          ...
          {email.length > 0 && (
            <XCircle
              className="h-[46px] w-11 px-3 stroke-custom-text-300 hover:cursor-pointer text-xs"
              onClick={() => {
                setEmail("");
        ...
        {emailError?.email && !isFocused && (
          <p className="flex items-center gap-1 text-xs text-red-600 font-medium px-0.5 pt-1">
      ...
      <Button type="submit" variant="primary" className="w-full" size="lg" disabled={isButtonDisabled}>
        {isSubmitting ? <Spinner height="20px" width="20px" /> : "Continua"}
      </Button>
    </form>
  );
});