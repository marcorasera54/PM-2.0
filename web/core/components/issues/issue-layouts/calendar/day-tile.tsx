...
export const CalendarDayTile: React.FC<Props> = observer((props) => {
  ...
  // Decide if is weekend set 100 or 90 so gray dark or light the entire background
  const normalBackground = isWeekend ? "bg-custom-background-100" : "bg-custom-background-100";
  const draggingOverBackground = isWeekend ? "bg-custom-background-80" : "bg-custom-background-90";

  return (
    <>
      <div ref={dayTileRef} className="group relative flex h-full w-full flex-col bg-custom-background-100">
        {/* header */}
        <div
          className={`hidden flex-shrink-0 items-center justify-end px-2 py-1.5 text-right text-xs md:flex gap-1 ${
